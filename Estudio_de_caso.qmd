...
## Datos Base (CFE) e Indicadores de Confiabilidad Eléctrica {#sec-datos-cfe}

El objetivo de esta fase es caracterizar cuantitativamente la tasa de ocurrencia y la escala temporal de las interrupciones del suministro eléctrico en el área geográfica de Tuxtla Gutiérrez, Chiapas. Los datos primarios provienen de los registros oficiales de la Comisión Federal de Electricidad (CFE) [@cfe2024informe], obtenidos mediante solicitudes de información pública.

De acuerdo con la normativa internacional estandarizada por el *Institute of Electrical and Electronics Engineers* en la norma IEEE Std 1366 [@sener2024prodesen], el desempeño y la continuidad de los sistemas de distribución eléctrica se evalúan cuantitativamente a través de dos índices fundamentales de confiabilidad:

### 1. Índice de Frecuencia de Interrupción Promedio por Usuario (SAIFI)
El indicador SAIFI (*System Average Interruption Frequency Index*) contabiliza el número promedio de interrupciones sostenidas que experimenta un usuario del sistema durante un periodo determinado (generalmente un año o un mes). Considerese una red eléctrica con un total de $N_T$ usuarios atendidos. Si en un intervalo de observación ocurren $M$ eventos aleatorios de interrupción, donde cada evento $i \in \{1, 2, \dots, M\}$ afecta a una cantidad de $N_i$ usuarios, el índice SAIFI se expresa formalmente como:

$$
\text{SAIFI} = \frac{\sum_{i=1}^{M} N_i}{N_T}
$$ {#eq-saifi}

### 2. Índice de Duración Promedio de Interrupción por Usuario (SAIDI)
El indicador SAIDI (*System Average Interruption Duration Index*) mide la duración total acumulada de las interrupciones sostenidas sufridas por el usuario promedio dentro del periodo de evaluación. Si cada evento de falla $i$ tiene una duración temporal de $r_i$ horas, el índice SAIDI se define analíticamente mediante:

$$
\text{SAIDI} = \frac{\sum_{i=1}^{M} N_i r_i}{N_T}
$$ {#eq-saidi}

### 3. Relación Cinética: Duración Promedio por Evento (CAIDI)
Para conectar la frecuencia de fallos con su duración individual dentro de la modelación estocástica, se define el índice CAIDI (*Customer Average Interruption Duration Index*), el cual representa el tiempo promedio de restablecimiento del servicio por cada evento de falla. A partir de las @eq-saifi y @eq-saidi, este indicador se deduce como el cociente directo entre el SAIDI y el SAIFI:

$$
\text{CAIDI} = \frac{\text{SAIDI}}{\text{SAIFI}} = \frac{\sum_{i=1}^{M} N_i r_i}{\sum_{i=1}^{M} N_i}
$$ {#eq-caidi}

### Análisis del Registro Histórico Municipal (2019-2025)
A partir de la información proporcionada por la CFE, la @tbl-interrupciones-cfe condensa el comportamiento mensual promedio de las interrupciones y su duración unitaria ($\text{CAIDI}$) registradas en el municipio de Tuxtla Gutiérrez durante el periodo 2019–2025.

| Año | Número de interrupciones promedio por mes ($\text{SAIFI}_{mensual}$) | Duración Promedio por evento $\text{CAIDI}$ (Horas) |
|:---:|:---:|:---:|
| 2019 | 25 | 2.02 |
| 2020 | 42 | 1.73 |
| 2021 | 47 | 2.03 |
...
: Interrupciones eléctricas mensuales y duración promedio en Tuxtla Gutiérrez, Chiapas (Fuente: Solicitud de Transparencia a la CFE). {#tbl-interrupciones-cfe}

El análisis de la @tbl-interrupciones-cfe muestra un incremento estructural significativo en la frecuencia de fallas a partir del año 2020, estabilizándose en una franja superior a los 40 cortes mensuales. Este cambio obedece al crecimiento de la carga térmica residencial e industrial y a la saturación de los transformadores de distribución en la mancha urbana.

Para la calibración de la EDE en esta investigación, se selecciona el año **2024** como el escenario de línea base. Dicho año presenta una media mensual representativa de **43 cortes por mes** con una duración promedio por evento de **2.39 horas**. Al proyectar esta tasa mensual sobre un horizonte anual completo de 12 meses, la variable discreta de interrupciones totales esperadas a nivel municipal ($N_{total}$) queda expresada como:
