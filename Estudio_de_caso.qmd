...
* **$\gamma$:** La magnitud del choque térmico instantáneo provocado por la entrada de calor exterior cada vez que se abre el equipo.
* **$dN_{puerta}(t)$:** Los eventos discretos que modelan el número de veces que el personal médico abre la puerta del refrigerador.
...
De acuerdo con las normativas internacionales de manejo de la red de frío, el refrigerador debe abrirse idealmente un máximo de dos veces al día: una apertura rutinaria matutina (típicamente a las 8:00 hrs) para extraer los frascos necesarios para la jornada, y una vespertina para resguardar los sobrantes. 

Dado que la apertura de las 8:00 hrs es un evento operativo determinista y predecible, la estocasticidad del sistema recae en el resto de la jornada, condicionada por la variabilidad de la demanda clínica. Para aproximar el modelo matemático a la realidad y aislar el factor humano, el Proceso de Poisson se segmenta en tres escenarios de simulación, definiendo tasas de intensidad ($\lambda$) dependientes del tiempo:

*   **Escenario A (Control Absoluto):** Simula días de inactividad clínica, como fines de semana o días festivos. La tasa del proceso estocástico es nula en todo momento ($\lambda = 0$). Las variaciones de la temperatura interna dependen exclusivamente de los cortes eléctricos y la temperatura ambiente exterior.
*   **Escenario B (Apego Normativo):** Respeta la única apertura extra permitida por la norma para resguardar biológicos sobrantes. Esta perturbación se modela como un Proceso de Poisson con una tasa de $1$ evento/día. Operativamente, esta tarea se realiza al final de la jornada de consulta. Al distribuir esta probabilidad en una ventana vespertina de 2 horas (de 15:00 a 17:00 hrs), la intensidad local se calcula como:
    $$\lambda_{tarde} = \frac{1 \text{ evento}}{2 \text{ horas}} = 0.5 \text{ eventos/hora}$$
    Fuera de este horario, la intensidad del proceso estocástico es estrictamente cero.
*   **Escenario C (Estrés Operativo y Contingencia):** Modela la saturación clínica donde la demanda sobrepasa la capacidad de los termos auxiliares de las enfermeras, obligando a realizar "viajes de resurtido" al refrigerador principal. Con base en los cercos epidemiológicos registrados en Chiapas durante 2025, el volumen de aplicación superó las 415,000 dosis mensuales, triplicando el consumo regular (aprox. 130,000 dosis). 
    Al escalar operativamente este incremento, la necesidad de extracción matutina se triplica (1 apertura fija a las 8:00 hrs + 2 aperturas estocásticas de resurtido). El retorno vespertino se mantiene normativo (1 apertura estocástica). Esto divide el proceso en dos ventanas activas:
...
### Impacto de la Apertura y Salto Térmico ($\gamma$)
...
Dado que en la práctica clínica intervienen múltiples factores humanos aleatorios e independientes, el Teorema del Límite Central justifica modelar la magnitud del salto $\gamma$ como una variable aleatoria con distribución Normal ($\mathcal{N}(\mu, \sigma^2)$). 
...
Para determinar la volatilidad, aplicamos la regla empírica (regla de las tres sigmas), asumiendo que el 99.7% de los eventos clínicos caerán dentro de los límites físicos observados. Igualando la distancia desde la media hasta el extremo superior con $3\sigma$:
$$3\sigma = 1.36 - 0.76 = 0.60 \implies \sigma = 0.20^\circ\text{C}$$

**Restricción Termodinámica (Filtro de Truncamiento):**
Las colas de una distribución Normal estándar se extienden hasta el infinito negativo. En el contexto físico, un valor $\gamma < 0$ implicaría que al abrir la puerta el refrigerador absorbe frío del exterior cálido, lo cual viola la Segunda Ley de la Termodinámica. Asimismo, el estudio del NIST establece que el ingreso mínimo absoluto de calor es de $0.16^\circ\text{C}$.

Para garantizar la viabilidad física del proceso estocástico durante las simulaciones de Montecarlo, la variable aleatoria se define formalmente como una distribución truncada inferiormente, aplicando un filtro de maximización:
